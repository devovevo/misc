...
TEST_F(LoadUnitTest, RandomMatrixLoadLifecycle) {
  ...
  // Evaluate purely combinational logic (IDLE state responds to fifo_empty_in
  // going low)
  dut->eval();
  ...
  // We now need to wait for read latency cycles to saturate the pipeline and
  // start reading
  ...
  for (int i = 0; i < SIZE; i++) {
    ...
    for (int j = 0; j < SIZE; j++) {
      EXPECT_EQ(top_shadow[j], W(SIZE - i - 1, j))
          << "Expected top shadow and W(SIZE - i - 1, j) to be equal with "
          << " i: " << i << " j: " << j << " whereas we actually"
          << " got top_shadow: " << top_shadow[j]
          << " and W(SIZE - i - 1, j): " << W(SIZE - i - 1, j);
    }
