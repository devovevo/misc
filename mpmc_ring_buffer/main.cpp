...
template <typename T, size_t n> class MPMCRingBuffer {
  ...
  bool push(T item) {
    ...
    while (true) {
      ...
      if (cur_seq == cur_head) {
        ...
        // Someone else incremented slot before we got there, so we need to look
        // at current head since it was probs stolen
