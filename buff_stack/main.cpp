...
public:
  ...
  bool push(T data) {
    ...
    while (true) {
      ...
      if (cur_seq == idx) {
        if (head.compare_exchange_weak(cur_head, next_head,
        ...
        // Someone else must have pushed here. We really want to check cur_seq >
        // idx, but since cur_seq < idx can't happen we just do an else
