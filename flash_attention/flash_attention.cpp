...
template <typename T>
void load(const T *A, T *B, const int M, const int N, const int m, const int n,
...
void flash_attention(const T *Q, const T *K, const T *V, T *O,
  ...
  T *q = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size * head_dim);
  T *k = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size * head_dim);
  T *v = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size * head_dim);
  T *s = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size * block_size);
  T *o = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size * head_dim);

  T *m = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size);
  T *l = (T *)std::aligned_alloc(std::hardware_constructive_interference_size,
                                 sizeof(T) * block_size);
  ...
  for (int block_i = 0; block_i < seq_len; block_i += block_size) {
    ...
    for (int block_j = 0; block_j < seq_len; block_j += block_size) {
      ...
      // Update our current output matrix with the new stuff (we'll add the
      // results after this)
      for (int i = 0; i < block_size; i++) {
...
int main() {
  ...
  float *Q =
      (float *)aligned_alloc(std::hardware_constructive_interference_size,
                             sizeof(float) * seq_len * head_dim);
  float *K =
      (float *)aligned_alloc(std::hardware_constructive_interference_size,
                             sizeof(float) * seq_len * head_dim);
  float *V =
