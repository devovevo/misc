...
template <typename T, const int BLOCK_SIZE_M, const int BLOCK_SIZE_N,
          const int PIPELINE_LEN>
__global__ void flash_attention(const T *Q, const T *K, const T *V, T *O,
  ...
  __shared__ cuda::pipeline_shared_state<cuda::thread_scope_block, 1>
      shared_q_load_pipeline_state;
  auto q_pipe = cuda::make_pipeline(block, &shared_q_load_pipeline_state);

  __shared__ cuda::pipeline_shared_state<cuda::thread_scope_block, PIPELINE_LEN>
      shared_k_load_pipeline_state;
  auto k_pipe = cuda::make_pipeline(block, &shared_k_load_pipeline_state);

  __shared__ cuda::pipeline_shared_state<cuda::thread_scope_block, PIPELINE_LEN>
      shared_v_load_pipeline_state;
  ...
  for (int block_i = 0; block_i < seq_len; block_i += blockStride) {
    ...
    cuda::memcpy_async(block, q, global_block_q_ptr,
                       cuda::aligned_size_t<16>(q_tile_bytes), q_pipe);
    ...
    wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, T, wmma::col_major>
        k_frag;
    wmma::fragment<wmma::matrix_b, WMMA_M, WMMA_N, WMMA_K, T, wmma::row_major>
        v_frag;
        ...
        for (int warp_j = 0; warp_j < BLOCK_SIZE_N; warp_j += WMMA_N) {
          ...
          for (int warp_k = 0; warp_k < head_dim; warp_k += WMMA_K) {
            wmma::load_matrix_sync(
                q_frag, q + (warp_row_offset + warp_i) * head_dim + warp_k,
                head_dim);
            wmma::load_matrix_sync(k_frag, k + (warp_j * head_dim) + warp_k,
                                   head_dim);
          ...
          T *warp_write_ptr =
              k_curr + (warp_row_offset + warp_i) * head_dim + warp_j;
          wmma::store_matrix_sync(warp_write_ptr, acc_s, BLOCK_SIZE_N,
                                  wmma::mem_row_major);
        }
      }
      ...
      for (int warp_i = 0; warp_i < WARP_SIZE; warp_i += WMMA_M) {
        for (int warp_j = 0; warp_j < head_dim; warp_j += WMMA_N) {
          for (int warp_k = 0; warp_k < BLOCK_SIZE_N; warp_k += WMMA_K) {
            wmma::load_matrix_sync(
                k_frag, k + (warp_row_offset * head_dim) + warp_k, head_dim);
            wmma::load_matrix_sync(v_frag, k + (warp_j * head_dim) + warp_k,
                                   head_dim);
